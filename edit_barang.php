...
try {
    ...
    $stmt = $conn->prepare("UPDATE Barang SET kategori_id = ?, brand_id = ?, ukuran_id = ?, rasa_id = ?, nama_barang = ?, harga_jual = ?, stok = ? WHERE barang_id = ?");
    $stmt->bind_param("iiiisdii", $kategori_id, $brand_id, $ukuran_id, $rasa_id_value, $nama_barang, $harga_jual, $stok, $barang_id);
    $stmt->execute();
    
    if ($stmt->affected_rows == 0) {
        throw new Exception("Gagal mengupdate barang. Tidak ada perubahan data.");
    }
    
    $stmt->close();
    $conn->commit();
    
    header('Location: data_barang.php?success=' . urlencode('Barang "' . $nama_barang . '" berhasil diupdate'));
    exit();
