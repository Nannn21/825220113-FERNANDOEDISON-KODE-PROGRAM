...
$current_page = 'kategori_barang.php';
$user = getCurrentUser();
...
$count_sql = "SELECT COUNT(*) as total FROM KategoriBarang $where_clause";
...
while ($row = $result->fetch_assoc()) {
    ...
    $count_stmt = $conn->prepare("SELECT COUNT(*) as row_num FROM KategoriBarang WHERE kategori_id <= ?");
    $count_stmt->bind_param("i", $row['kategori_id']);
    $count_stmt->execute();
...
<head>
    ...
    <title>Kategori Barang - Inventaris Barang</title>
...
<body>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th style="width: 60%;">Kategori Barang</th>
                                ...
                                <tr>
                                    <td colspan="3">
                                        <div class="empty-state">
                                            ...
                                            <p>Tidak ada data kategori<?php echo !empty($search) ? ' yang sesuai dengan pencarian' : ''; ?></p>
                                        </div>
                                    </td>
                                </tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    <td><?php echo $kategori['row_num']; ?></td>
                                    <td><?php echo htmlspecialchars($kategori['nama_kategori']); ?></td>
                                    <td>
                                        <button class="btn-edit" onclick="openEditModal(<?php echo $kategori['kategori_id']; ?>)">
                                            Edit
                                        </button>
                                        <button class="btn-hapus" onclick="confirmDeleteKategori(<?php echo $kategori['kategori_id']; ?>, '<?php echo addslashes($kategori['nama_kategori']); ?>')">
                                            Hapus
                                        </button>
                                    </td>
                                </tr>
    ...
    <div id="editModalOverlay" class="modal-overlay" onclick="closeEditModalOnOverlay(event)">
        <div class="modal-box" onclick="event.stopPropagation()">
            ...
            <form id="formEditKategori" method="POST" action="edit_kategori.php">
                
                <input type="hidden" id="edit_kategori_id" name="kategori_id" value="">
                ...
                <div class="form-group">
                    ...
                    <span class="error-message" id="error_edit_nama_kategori"></span>
                </div>
    ...
    <div id="modalOverlay" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box" onclick="event.stopPropagation()">
            <div class="modal-header">
                <h2>Tambahkan Kategori Barang</h2>
            ...
            <form id="formTambahKategori" method="POST" action="tambah_kategori.php">
                <div class="form-group">
                    ...
                    <input type="text" 
                           ...
                           required 
                           placeholder="Masukkan nama kategori">
                    <span class="error-message" id="error_nama_kategori"></span>
                </div>
    ...
    <script>
        ...
        function confirmDeleteKategori(kategoriId, namaKategori) {
            if (confirm('Apakah Anda yakin ingin menghapus kategori "' + namaKategori + '"?\n\nPeringatan: Kategori yang digunakan oleh barang tidak dapat dihapus.')) {
                window.location.href = 'hapus_kategori.php?id=' + kategoriId;
            }
        }
        ...
        if (formTambahKategori) {
            formTambahKategori.addEventListener('submit', function(e) {
                ...
                const inputs = formTambahKategori.querySelectorAll('input');
                inputs.forEach(function(el) {
                    el.classList.remove('error');
                });

                const namaKategori = document.getElementById('nama_kategori').value.trim();
                if (!namaKategori || namaKategori.length === 0) {
                    isValid = false;
                    document.getElementById('error_nama_kategori').textContent = 'Nama kategori tidak boleh kosong';
                    document.getElementById('nama_kategori').classList.add('error');
                } else if (namaKategori.length > 50) {
                    isValid = false;
                    document.getElementById('error_nama_kategori').textContent = 'Nama kategori maksimal 50 karakter';
                    document.getElementById('nama_kategori').classList.add('error');
                }
                ...
                if (!isValid) {
                    ...
                    const firstError = formTambahKategori.querySelector('.error');
                    if (firstError) {
                        firstError.focus();
                    }
                    return false;
                ...
                const submitButton = formTambahKategori.querySelector('button[type="submit"]');
                if (submitButton) {
        ...
        function openEditModal(kategoriId) {
            ...
            fetch('get_kategori_data.php?id=' + kategoriId)
                ...
                .catch(error => {
                    alert('Gagal mengambil data kategori: ' + error);
                });
        }
        ...
        function closeEditModal() {
            ...
            const errorMessages = document.querySelectorAll('#formEditKategori .error-message');
            errorMessages.forEach(function(el) {
                el.textContent = '';
            });
            ...
            if (formEditKategori) {
                const inputs = formEditKategori.querySelectorAll('input');
                inputs.forEach(function(el) {
                    el.classList.remove('error');
                });
            }
        ...
        if (formEditKategori) {
            formEditKategori.addEventListener('submit', function(e) {
                ...
                if (!namaKategori || namaKategori.length === 0) {
                    isValid = false;
                    document.getElementById('error_edit_nama_kategori').textContent = 'Nama kategori tidak boleh kosong';
                    document.getElementById('edit_nama_kategori').classList.add('error');
                } else if (namaKategori.length > 50) {
                    isValid = false;
                    document.getElementById('error_edit_nama_kategori').textContent = 'Nama kategori maksimal 50 karakter';
                    document.getElementById('edit_nama_kategori').classList.add('error');
                }
                ...
                if (!isValid) {
                    ...
                    const firstError = formEditKategori.querySelector('.error');
                    if (firstError) {
                        firstError.focus();
                    }
                    return false;
                ...
                const submitButton = formEditKategori.querySelector('button[type="submit"]');
                if (submitButton) {
                    submitButton.disabled = true;
                    submitButton.textContent = 'Menyimpan...';
                }
